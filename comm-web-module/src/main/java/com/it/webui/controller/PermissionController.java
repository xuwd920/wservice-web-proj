...
import com.it.webui.shiro.ShiroService;
...
public class PermissionController{
    ...
    @PostMapping("/list")
    @ResponseBody
    public List<SysPermission>  loadPermissions(String flag){
        List<SysPermission> permissionListList = new ArrayList<SysPermission>();
    ...
    @ResponseBody
    @PostMapping("/add")
    public ResponseVo addPermission(SysPermission permission){
        try {
    ...
    public String detail(Model model, String permissionId) {
        SysPermission permission = permissionService.findByPermissionId(permissionId);
        if(null!=permission){
            if(permission.getParentId().equals(CoreConst.TOP_MENU_ID)){
                model.addAttribute("parentName", CoreConst.TOP_MENU_NAME);
            }else{
                SysPermission parent = permissionService.findById(permission.getParentId());
    ...
    @ResponseBody
    @PostMapping("/edit")
    public ResponseVo editPermission(@ModelAttribute("permission")SysPermission permission){
